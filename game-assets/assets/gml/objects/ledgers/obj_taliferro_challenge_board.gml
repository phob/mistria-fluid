object_create(
    "obj_taliferro_challenge_board",
    object_reserve("par_interactable"),
    {
        sprite_index: spr_town_stall_taliferro_challenge_board_main,
        create: function() {
            event_inherit(ObjectEvent.Create);
            self.mask_index = spr_mask_requestboard;

            if SATURDAY_MARKET.stalls[NpcId.Taliferro] == false {
                instance_destroy();
                return;
            }
        ...
        draw_end: function() {
            if available_tali_challenge() == undefined || MIST.running {
                return;
            }
