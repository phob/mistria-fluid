# Nexus 2.3.0 update checklist

1. The manifest is final: author `phobi666`, version `2.3.0`,
...
3. Page details:
   ...
   - **Overview (short):** Move with the mouse, click characters and objects
     ...
     watering, tilling, bugs, breakables, and fishable water. Configure
     gameplay options in game. WASD always stays in control. Ready for Fields
     of Mistria 1.0; requires MOMI 0.15.2 or newer.
...
5. Files tab: upload `dist/ARPGMovement-2.3.0.zip`, version `2.3.0`.

6. Changelog (paste as plain text, one entry per line):

   ```text
   Added in-game ARPG Movement settings
   Placed ARPG settings directly above Exit
   Clicking fishable water now equips the fishing rod
   Dig spots use pickaxes when no shovel is carried
   Improved outside-click behavior for layered menus
   ```
...
8. Mark the 2.2.1 file as archived only after confirming the 2.3.0 download
